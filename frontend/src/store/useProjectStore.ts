...
interface ProjectState {
  ...
  exportEditablePPTXImg2Slides: () => Promise<void>;
}
...
const debouncedUpdatePage = debounce(
  ...
  // 导出可编辑PPTX（Vision识别版，使用img2slides）
  exportEditablePPTXImg2Slides: async () => {
    const { currentProject } = get();
    if (!currentProject) return;

    set({ isGlobalLoading: true, error: null });
    try {
      const response = await api.exportEditablePPTXImg2Slides(currentProject.id);
      const downloadUrl =
        response.data?.download_url || response.data?.download_url_absolute;
