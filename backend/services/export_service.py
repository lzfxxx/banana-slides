...
class ExportService:
    ...
    @staticmethod
    def create_editable_pptx_with_img2slides(
        ...
        """
        Create editable PPTX using img2slides vision analysis

        Args:
            image_paths: List of PNG image paths
            output_file: Optional output path
            provider: AI provider ("claude" or "gemini")
            api_key: Optional API key override
            base_url: Optional API base URL override
            model: Optional model override
            crop_padding: Crop padding percentage

        Returns:
            PPTX bytes if output_file is None
        """
        ...
        structures = []
        for image_path in image_paths:
            logger.info(f"Analyzing image with img2slides: {image_path}")

            # Analyze image (analyze_image handles provider creation internally)
        ...
        logger.info(f"Generating editable PPTX with {len(structures)} slides")
